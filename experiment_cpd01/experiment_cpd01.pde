...
int catch_answer_ms = 4000;
int trial_interval_ms = 1000;
...
// 画像をダウンロードする時に使用しました。
String [] fileter_correspondence_table = {"B","A"};
...
float min_cpd = 4.4;
// cpdの刻み幅
float cpd_stride = 0.1;
int cpd_num = round( ( (max_cpd - min_cpd) / cpd_stride ) + 1 );

// loadする画像の数を計算してます。２(バタワースかそうでないフィルタを用いているか) x (選択肢となるアルファベットは何種類あるか？) x (cpdはいくつあるか？)
...
void setup() {
  ...
  println("trial num : " + trial_num);
  println("experiment minute : " + (initial_nothing_display_ms + one_trial_ms * trial_num)/( 1000 * 60 ));
  ...
  println("use alphabets num : " +  use_alphabets.length);
  println("cpd num : " + cpd_num);
  println("filter num : " + fileter_correspondence_table.length);
  
  // 画像のダウンロードと同時にファイル名も保存してます。
  for(int alphabet_index = 0; alphabet_index < use_alphabets.length ; alphabet_index++){
    for(float cpd = min_cpd; cpd <= max_cpd ; cpd += cpd_stride){
      for(int filter = 0; filter < fileter_correspondence_table.length ; filter++){
        // 画像のダウンロード部分です。
        // floor() : 小数点以下を切り捨てる
        imgs[index_used_onlyWhen_ImgLoad] = loadImage(image_saved_folder+'/'+use_alphabets[alphabet_index]+"_"+str(floor(cpd))+"cpd_"+fileter_correspondence_table[filter]+"filter.png");
        // 画像をグレースケールに変更してます。
        imgs[index_used_onlyWhen_ImgLoad].filter(GRAY);
        // 画像のサイズを変更してます。
        char_size_mm = tan( (floor(cpd) / cpd) * ( PI / 180 ) ) * screen_to_eye_mm;
        ...
        ////////// 上のタイミングで保存して表示する時に、resizeしたサイズになるかどうかを極端な値にして検証しました。
        //if ( filter == 0 ){
        //  char_size_mm = 100.0;
        //  imgs[index_used_onlyWhen_ImgLoad].resize( int(char_size_mm*5), 0 );
        //}
